...
## R0-05B protection correction and verification

- Corrected and verified at `2026-08-30T14:28:02Z` through the GitHub API; no force-push, history rewrite, or bypass was used.
- Active rule: repository ruleset `protect-main-release-governance` (`21861412`), `target=branch`, `enforcement=active`, ref include `refs/heads/main`, exclude `[]`.
- Direct-change control: pull-request rule is active with `required_approving_review_count=0`; direct pushes are not an allowed governed merge path for `main`.
- Review control: zero approvals required by controlled autonomy; stale reviews dismissed on push; `bypass_actors=[]`, `current_user_can_bypass=never`.
...
- Required checks configured by the corrected ruleset: `safe-checks`, `Hosted quality gate`, `lighthouse` (GitHub Actions integration `15368`), `smoke`, `pnpm audit (high/critical gate)`, `Secret scan (repo)`, and `CodeQL` (integration `57789`).
...
- Effectiveness verdict: **complete**. Every required context is an exact, stable, always-emitted PR check on both governed PR heads; path-conditional deployment checks are excluded.
- PR #21 was marked ready and merged through the ruleset as merge commit `308c2753eabab74d1dc0031fd0e2b76a4fabee39`; no approval or bypass was used. PR #20 remains `OPEN`, `DRAFT`, `MERGEABLE`, `CLEAN`, with no approving review.
